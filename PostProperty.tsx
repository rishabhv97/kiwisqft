...
const PostProperty: React.FC<PostPropertyProps> = ({ onAddProperty, isAdmin = false }) => {
  const navigate = useNavigate();
  const [step, setStep] = useState<'selection' | 'form'>(isAdmin ? 'form' : 'selection'); // Skip selection for Admin
  ...
  const [formData, setFormData] = useState({
    ...
    listedBy: isAdmin ? 'Agent' : 'Owner' as ListedBy, // Default to Agent for Admin
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newProperty: Property = {
      ...
      ownerContact: isAdmin ? 'Admin Office' : 'Hidden for Demo',
      datePosted: new Date().toISOString(),
      isFeatured: isAdmin, // Admin posts are featured by default
      ...
      ownerId: isAdmin ? 'admin' : 'user-demo',
    };

    setTimeout(() => {
      onAddProperty(newProperty);
      setIsSubmitting(false);
      // Redirect based on who posted
      if (isAdmin) {
  ...
  // 1. SELECTION SCREEN (Only show if NOT admin)
  if (step === 'selection' && !isAdmin) {
    return (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-5xl w-full">
          ...
          >
            ...
            <p className="text-gray-600 mb-8 leading-relaxed text-lg">
              Partner with top-rated local agents.
            </p>
          ...
          >
            ...
            <p className="text-gray-600 mb-8 leading-relaxed text-lg">
              Create a professional listing in minutes.
            </p>
  ...
  // 2. FORM SCREEN
  return (
    <div className={`min-h-screen ${isAdmin ? 'bg-gray-100 p-6' : 'bg-brand-beige py-12'}`}>
      <div className={`${isAdmin ? 'max-w-7xl' : 'max-w-4xl'} mx-auto ${!isAdmin && 'px-4'}`}>
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-brand-green/10">
          ...
          <div className="bg-brand-brown p-8 text-center text-white relative">
            ...
            <h1 className="text-3xl font-bold mb-2">{isAdmin ? 'Admin Property Post' : 'Post Your Property'}</h1>
            <p className="text-brand-lightGreen/80">{isAdmin ? 'Properties posted here are automatically approved.' : 'Detailed listings attract 3x more buyers.'}</p>
          </div>

          <form onSubmit={handleSubmit} className="p-8 space-y-10">
            {/* ... EXISTING FORM CONTENT SAME AS BEFORE, JUST RENDERED ... */}
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                   ...
                   <select name="ownershipType" value={formData.ownershipType} onChange={handleChange} className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-green focus:border-transparent outline-none bg-white">
                ...
                <div>
                  ...
                  <select name="type" value={formData.type} onChange={handleChange} className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-green focus:border-transparent outline-none bg-white">
                 ...
                 <div>
                   ...
                   <select name="listedBy" value={formData.listedBy} onChange={handleChange} className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-green focus:border-transparent outline-none bg-white">
                 ...
                 <div className="md:col-span-2">
                   ...
                   <input type="text" name="city" value={formData.city} onChange={handleChange} placeholder="e.g. Noida" required className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-green outline-none"/>
                </div>
                 <div className="md:col-span-2">
                   <label className="block text-sm font-medium text-gray-700 mb-1">Locality / Area</label>
                   <input type="text" name="location" value={formData.location} onChange={handleChange} placeholder="e.g. Sector 62" required className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-green outline-none"/>
                </div>
              </div>
            </div>
            ...
            <div>
               <div className="flex items-center gap-2 mb-6 border-b border-gray-100 pb-2">
                 ...
                 <h2 className="text-xl font-bold text-gray-800">Room & Area Details</h2>
              </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div>
                            ...
                            <div className="flex gap-2">
                                {[1, 2, 3, 4, 5].map(n => (
                                    <button key={n} type="button" onClick={() => handlePillSelect('bedrooms', n)} className={`w-10 h-10 rounded-full font-semibold border transition-all ${formData.bedrooms === n ? 'bg-brand-green text-white border-brand-green' : 'bg-white text-gray-600 border-gray-200 hover:border-brand-green'}`}>{n}{n===5 ? '+' : ''}</button>
                                ))}
                            </div>
                        </div>
                        <div>
                            ...
                            <div className="flex gap-2">
                                {[1, 2, 3, 4].map(n => (
                                    <button key={n} type="button" onClick={() => handlePillSelect('bathrooms', n)} className={`w-10 h-10 rounded-full font-semibold border transition-all ${formData.bathrooms === n ? 'bg-brand-green text-white border-brand-green' : 'bg-white text-gray-600 border-gray-200 hover:border-brand-green'}`}>{n}{n===4 ? '+' : ''}</button>
                                ))}
                            </div>
                        </div>
                        <div>
                            ...
                            <div className="flex gap-2">
                                {[0, 1, 2, 3].map(n => (
                                    <button key={n} type="button" onClick={() => handlePillSelect('balconies', n)} className={`w-10 h-10 rounded-full font-semibold border transition-all ${formData.balconies === n ? 'bg-brand-green text-white border-brand-green' : 'bg-white text-gray-600 border-gray-200 hover:border-brand-green'}`}>{n}{n===3 ? '+' : ''}</button>
                                ))}
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="flex flex-col gap-4 mt-4">
                            <div className="border border-gray-300 rounded-lg overflow-hidden flex items-center bg-white hover:border-brand-green transition-colors">
                                <div className="flex-1 relative">
                                    ...
                                    <input type="number" name="superBuiltUpArea" value={formData.superBuiltUpArea} onChange={handleChange} className="w-full pt-7 pb-2 px-3 outline-none text-gray-800 font-medium" />
                                </div>
              ...
              <div className="space-y-6 mb-8">
                    <div className="border border-gray-300 rounded-lg overflow-hidden flex items-center bg-white hover:border-brand-green transition-colors">
                        <div className="flex-1 relative">
                            ...
                            <input type="number" name="price" value={formData.price} onChange={handleChange} className="w-full pt-7 pb-2 px-3 outline-none text-gray-800 font-medium text-lg" placeholder="Enter total price"/>
                        </div>
                    </div>
                    {formData.price && (
            ...
            <div>
               <div className="flex items-center gap-2 mb-6 border-b border-gray-100 pb-2">
                ...
                <div className="md:col-span-2 mb-6">
                   ...
                   <input type="text" name="title" value={formData.title} onChange={handleChange} placeholder="e.g. Luxury 3BHK Apartment in Sector 150" required className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-green outline-none"/>
                </div>

              <div className="relative mb-6">
                <textarea name="description" value={formData.description} onChange={handleChange} rows={5} className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-green outline-none" placeholder="What makes your property unique?"></textarea>
                <button type="button" onClick={handleGenerateDescription} disabled={isGenerating} className="absolute bottom-3 right-3 bg-purple-600 hover:bg-purple-700 text-white text-xs font-bold py-2 px-3 rounded-md flex items-center gap-1 transition-colors disabled:opacity-50 shadow-md">
            ...
            <div className="flex justify-end gap-4 border-t border-gray-100 pt-6">
              <button type="button" onClick={() => isAdmin ? navigate('/admin/properties') : navigate('/')} className="px-6 py-3 border border-gray-300 rounded-lg text-gray-700 font-medium hover:bg-gray-50">Cancel</button>
              <button type="submit" disabled={isSubmitting} className="px-8 py-3 bg-brand-green text-white rounded-lg font-bold hover:bg-emerald-800 transition-all flex items-center gap-2 disabled:opacity-70 shadow-lg shadow-brand-green/20">
                {isSubmitting ? <><Loader2 size={20} className="animate-spin" /> Posting...</> : <><CheckCircle size={20} /> {isAdmin ? 'Approve & Post' : 'Post Property'}</>}
              </button>
            </div>
          </form>
        </div>
