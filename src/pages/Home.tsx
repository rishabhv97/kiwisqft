...
interface SellerProfile {
  ...
  role: string;
  image?: string;
  // We will generate these for UI demo since they aren't in DB yet
  experience: string;
...
const Home: React.FC<HomeProps> = ({ featuredProperties }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // A. Fetch Sellers (Profiles with role 'Seller' or 'Broker')
        ...
        // B. Calculate Popular Localities from Actual Properties
        const { data: allProps } = await supabase
          .from('properties')
          .select('location, price, image_url');
        ...
        if (allProps && allProps.length > 0) {
          ...
          const locMap = allProps.reduce((acc: any, curr: any) => {
            ...
            if (!acc[loc]) {
              acc[loc] = { count: 0, totalPrice: 0, image: curr.image_url };
            }
            acc[loc].count += 1;
          ...
          const derivedLocalities = Object.keys(locMap).map(key => ({
            ...
            image: locMap[key].image || 'https://images.unsplash.com/photo-1560518883-ce09059eeffa?auto=format&fit=crop&w=600&q=80'
          })).sort((a, b) => b.count - a.count).slice(0, 8); // Top 8

          setLocalities(derivedLocalities);
        } else {
            // Fallback if DB is empty (so the UI isn't broken)
            setLocalities([
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    if (activeTab === 'sell') {
        ...
        const type = activeTab === 'rent' ? 'rent' : 'sale';
        navigate(`/${activeTab}?search=${searchTerm}`);
    }
  };

  const formatPrice = (price: number) => {
    return `₹${(price / 100000).toFixed(1)}L`; // Approx formatted price
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-16 mb-8">
        ...
        <div className="flex overflow-x-auto pb-6 gap-5 snap-x snap-mandatory -mx-4 px-4 sm:mx-0 sm:px-0">
          {localities.length > 0 ? localities.map((loc) => (
            <div key={loc.name} onClick={() => navigate(`/buy?search=${loc.name}`)} className="min-w-[200px] sm:min-w-[240px] md:min-w-[260px] flex-shrink-0 snap-start group bg-white rounded-xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col cursor-pointer">
              ...
              <div className="p-4 flex flex-col justify-between flex-grow">
                 <div>
                    ...
                    <p className="text-sm text-gray-600 font-medium">{loc.count}+ Properties</p>
                 </div>
              </div>
            </div>
          )) : (
            <div className="text-gray-400 italic p-4">Loading localities...</div>
          )}
        </div>
      </div>
          ...
          {sellers.length > 0 ? sellers.map((seller) => (
             <div key={seller.id} className="min-w-[85vw] sm:min-w-0 bg-white rounded-xl border border-gray-200 p-5 snap-center hover:shadow-lg transition-shadow flex flex-col">
                ...
                <div className="flex items-center gap-4 text-sm text-gray-600 mb-5 bg-gray-50/50 p-3 rounded-lg border border-gray-100">
                   ...
                   <div><span className="font-bold text-gray-900 text-lg">{seller.propertiesCount}</span> <span className="text-xs">Properties</span></div>
                </div>

                {/* Localities */}
                <div className="flex flex-wrap gap-2 mb-6 flex-grow">
                   {seller.localities.map((loc, i) => (
                      <span key={i} className="bg-white border border-gray-200 text-gray-600 text-xs font-medium px-3 py-1.5 rounded-full shadow-sm">
                         {loc}
                      </span>
                   ))}
                </div>
             ...
             [1, 2, 3].map(n => (
               <div key={n} className="h-48 bg-gray-100 animate-pulse rounded-xl"></div>
             ))
          )}
        </div>
      </div>
